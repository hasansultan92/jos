...
boot_alloc(uint32_t n)
{
	...
	char *result;
	cprintf("%s: setting up next free pointer\n", __func__);
...
mem_init(void)
{
	...
	// LAB 3: Your code here.
	size_t env_region_size = ROUNDUP(sizeof(struct Env)  * NENV, PGSIZE);
	boot_map_region(kern_pgdir, UENVS, env_region_size, PADDR(envs), PTE_U | PTE_P);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	// Get end addr [va, va+len] with permissions
